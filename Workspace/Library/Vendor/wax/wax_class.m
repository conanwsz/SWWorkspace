...
static NSString *wax_originalAllocWithZoneStr = @"wax_originalAllocWithZone:";
...
static int __call(lua_State *L) {   
    ...
    if (!klass) {
        ...
        if (method_getImplementation(m) != (IMP)allocWithZone) {
            SEL sel_wax_originalAllocWithZone = NSSelectorFromString(wax_originalAllocWithZoneStr);
            class_addMethod(metaclass, sel_wax_originalAllocWithZone, method_getImplementation(m), method_getTypeEncoding(m));
...
//    id instance = [self wax_originalAllocWithZone:zone];
    SEL sel_wax_originalAllocWithZone = NSSelectorFromString(wax_originalAllocWithZoneStr);
    id instance = [self performSelector:sel_wax_originalAllocWithZone withObject:(id)zone];
