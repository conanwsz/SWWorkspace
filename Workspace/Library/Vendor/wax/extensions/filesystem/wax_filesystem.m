...
static int contents(lua_State *L) {
    ...
    if (!exists) {
        ...
        if (error) {
            wax_log(WAX_LOG_DEBUG, @"Could not get contents for directory at '%@'\n%@", path, [error localizedDescription]);
        }
...
static int attributes(lua_State *L) {
    ...
    if (error) {
        wax_log(WAX_LOG_DEBUG, @"No attributes found for '%@'\n%@", path, [error localizedDescription]);
        lua_pushnil(L);
        return  1;
    }
...
static int createFile(lua_State *L) {
    ...
    if (!success) {
        wax_log(WAX_LOG_DEBUG, @"Could not create file at '%@'\n%@", path, [error localizedDescription]);
    }
...
static int createDir(lua_State *L) {
    ...
    if (!success) {
        wax_log(WAX_LOG_DEBUG, @"Could not create dir at '%@'\n%@", path, [error localizedDescription]);
    }
...
static int delete(lua_State *L) {
	...
	if ([fm fileExistsAtPath:path]) {	
		...
		if (!success) {
			wax_log(WAX_LOG_DEBUG, @"Could not delete file at '%@'\n%@", path, [error localizedDescription]);
		}
	}
	else {
		wax_log(WAX_LOG_DEBUG, @"Trying to delete path that does not exist '%@'", path);
	}
